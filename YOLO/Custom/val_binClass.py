...
#             preds.extend((outputs > 0.5).cpu().numpy())  # Binary predictions
...
#     LOGGER.info(
#         f"Validation Loss: {total_loss:.4f}, Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}"
#     )
...
def run(
    ...
    model.eval()
    preds, targets, total_loss = [], [], 0
    desc = "Validating" if dataloader.dataset.root.stem == "val" else "Testing"
    bar = tqdm(dataloader, desc=desc, total=len(dataloader), bar_format=TQDM_BAR_FORMAT, position=0)
    with torch.cuda.amp.autocast(enabled=device.type == "cuda"):
        for images, labels in bar:
            ...
            preds.extend((outputs > 0.5).float().cpu().numpy())  # Binary predictions
            targets.extend(labels.cpu().numpy())
    ...
    # Metrics calculation
    preds = [p[0] for p in preds]  # Flatten predictions
    accuracy = (preds == targets).mean()
    precision = precision_score(targets, preds, zero_division=1)
    recall = recall_score(targets, preds, zero_division=1)
    f1 = f1_score(targets, preds, zero_division=1)
    ...
    # Log results
    total_loss /= len(dataloader)
    LOGGER.info(f"Validation Loss: {total_loss:.4f}, Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}")

    # Save results
    if not training:
        results_path = save_dir / "validation_results.txt"
        with open(results_path, "w") as f:
            f.write(f"Validation Loss: {total_loss:.4f}\n")
